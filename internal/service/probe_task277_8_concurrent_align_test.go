...
func TestConcurrentAlignmentsSerialized(t *testing.T) {
	svc := newTestService(t)
	b := mustShellBatch(t, svc, "C-ALIGN")
	samples, err := svc.ListSamples(b.ID)
	if err != nil {
		t.Fatal(err)
	}
	const workers = 20
	...
	if len(as) != len(samples) {
		t.Fatalf("alignments=%d samples=%d", len(as), len(samples))
	}
}
