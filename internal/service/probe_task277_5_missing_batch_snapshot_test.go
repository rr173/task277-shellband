...
func TestBuildSnapshotMissingBatchNoPanic(t *testing.T) {
	svc := newTestService(t)
	defer func() {
		if rec := recover(); rec != nil {
			t.Fatalf("BuildSnapshot panicked: %v", rec)
		}
	}()
	_, err := svc.BuildSnapshot(999, false)
	if err == nil {
		t.Fatal("missing batch must return an error")
	}
}
